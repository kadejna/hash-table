...
private:
    ...
    // a hash table is the array of the buckets (linked lists), in which where the key/value are stored
    Node** hashTable; 
    // how many key/value am i storing right now
    unsigned int sz; 
    // how many key/value can i store it in the hash map (number of buckets in hash map)
    unsigned int capacity; 
    // this is a rehash function used in the add function, when load factor reaches and surpasses 0.8
    void reHash(); 
};  
